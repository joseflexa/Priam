...
public class SSTableLoaderWrapper {
	private static final Logger logger = LoggerFactory
			.getLogger(SSTableLoaderWrapper.class);
	private static Set<Component> allComponents = Sets.newHashSet(
			Component.COMPRESSION_INFO, Component.DATA, Component.FILTER,
	...
	public SSTableLoaderWrapper(IConfiguration config) throws IOException {
		...
		TuneCassandra.updateYaml(config, url.getPath(), "localhost",
	...
	public void stream(File directory) throws IOException, InterruptedException {
		...
		for (SSTableReader sstable : sstables) {
			...
			List<Pair<Long, Long>> sections = Lists
					.newArrayList(new Pair<Long, Long>(0L, sstable
							.onDiskLength()));
			PendingFile pending = new PendingFile(sstable, desc,
					SSTable.COMPONENT_DATA, sections, OperationType.BULK_LOAD,
					sstable.estimatedKeys());
			StreamHeader header = new StreamHeader(directory.getName(),
					System.nanoTime(), pending, Collections.singleton(pending));
			logger.info("Streaming to {}", InetAddress.getLocalHost());
			EncryptionOptions options = new EncryptionOptions();
			new FileStreamTask(header, InetAddress.getLocalHost(), options)
					.run();
	...
	public void deleteCompleted() throws IOException {
		logger.info("Restored SST's Now Deleting: "
				+ StringUtils.join(pendingDeletion, ","));
